...
#    $ python3 main.py --container magika --registry all --prune-untagged-age 7 --dry-run --verbose
#
# 2. Delete ALL untagged images from GHCR only:
#    $ python3 main.py --container magika --registry ghcr --prune-all-untagged --verbose
#
# 3. Keep only the latest 3 images in Docker Hub (delete everything else):
#    $ python3 main.py --container magika --registry dockerhub --keep-latest 3 --verbose
#
# 4. Aggressive cleanup - keep only latest 2 images across both registries:
#    $ python3 main.py --container magika --registry all --keep-latest 2 --verbose
#
# 5. Clean up old development images (30+ days, untagged only):
#    $ python3 main.py --container magika --registry all --prune-untagged-age 30 --verbose
#
# 6. Preview what would be deleted without making changes:
#    $ python3 main.py --container magika --registry all --keep-latest 5 --dry-run --verbose
#
...
Examples:
  ...
  python3 main.py --container magika --registry ghcr --prune-untagged-age 7 --dry-run

  # Prune all untagged images from Docker Hub
  python3 main.py --container magika --registry dockerhub --prune-all-untagged

  # Keep only the latest 3 images in both registries
  python3 main.py --container magika --registry all --keep-latest 3
